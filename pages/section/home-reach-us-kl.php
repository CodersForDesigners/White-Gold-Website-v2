<section class="reach-us space-100-bottom space-200-top" id="contact-us">

    <div class="container">
    <div class="text-center mb-4">
            <h2>Contact Us</h2>
        </div>   
<div class="row mb-5">
<div class="widget-area columns small-12 medium-12 large-10 pd-10 mb-4">
                        <aside class="widget widget-text cmt-bgcolor-skincolor pd1030 item4">
                            <!--featured-icon-box-->
                            <div class="featured-icon-box icon-align-before-content">
                                <div class="featured-icon">
                                    <div class="cmt-icon cmt-icon_element-fill cmt-icon_element-color-darkgrey cmt-icon_element-size-sm cmt-icon_element-style-square">
                                    <img src="/content/cms/Edappally2..jpg" class="img-fluid" alt="whitegold">
                                    </div>
                                </div>
                                <div class="featured-content">
                                    <div class="featured-title">
				    <h4 class="mb-2">White Gold Edappally</h4>
                                   <h5 class="strong mb-2 mt-2 d-block">Registered Address:</h5>
                                   <a href="https://maps.app.goo.gl/yVZeaLdnVotojG9m7" target="_blank"><p class="address d-flex mb-3 mt-2"> C2, Fourth Floor, Vellakkal VAM Arcade, No 33/195, nearby Vishal Mega Mart, Toll Junction, Edappally, Kochi, Kerala 682024</p></a>
<a href="tel:9590704444"><p class="number d-flex">+91 95622 62222</p></a>

                                </div>
                                    <div class="featured-desc mb-3 mt-2">
                                    
									</div>
                                </div>
                            </div><!-- featured-icon-box end-->
                        </aside>
					</div>
                    <div class="col-lg-1 col-md-12 col-xs-12"></div></div>
                    </div>
                    </section>
<style>
/* start reach us  */

.d-block{
display:block;
}

  .item4{
    padding:10px;
    background-color:white;
    box-shadow: 0px 0px 33.9723px rgba(0, 50, 160, 0.1);
    border-radius: 15.9869px;
    position: relative;
}

/* .reach-us  h2{
font-size:32px;
font-weight:700;
}
.reach-us h4{
    font-size:24px;
    font-weight:700;
    display: table-cell;
    vertical-align: middle;
} */

.reach-us span{
    font-weight: 500;
    color: #00000075;
    display: table-cell;
    vertical-align: middle;
}
.reach-us .row{
display: flex;
    align-items: center;
    justify-content: center;
}


/* .reach-us p {
    display: inline-block;
    font-size: 16px;
    color: #212322;
} */

.widget-area .widget {
    padding: 7px 13px;
    position: relative;
}
.featured-icon-box {
    position: relative;
    margin: 15px 0;
}

.featured-icon-box.icon-align-before-content .featured-content, .featured-icon-box.icon-align-before-title .featured-title {
    padding-left: 15px;
}

.reach-us .address::before{
    margin-right: 12px;
    content:url('https://staging.whitegold.money/cms/../content/cms/gps-2-1-1.svg');  
}

.reach-us .number::before{
    position: relative;
    top: -5px;
    margin-right: 13px;
    content:url('https://staging.whitegold.money/cms/../content/cms/telephone-1-1.svg');  
}
 
@media (max-width: 480px){
    .featured-icon-box.icon-align-before-content .featured-icon, .featured-icon-box.icon-align-before-content .featured-content, .featured-icon-box.icon-align-before-title .featured-title, .featured-icon-box.icon-align-before-title .featured-icon {
        padding-top: 0px;
        display: block;
    }

    .reach-us  h2{
font-size:32px;
font-weight:700;
}
.reach-us h4{
    font-size:16px;
    font-weight:700;
    display: table-cell;
    vertical-align: middle;
}
.reach-us .container {
    max-width: 380px!important;
}
.reach-us p {
    display: block;
    font-size: 14px;
    color: #212322;
}

}
@media (min-width: 1200px){
.reach-us .featured-title{
padding-right: 50px;
    padding-left: 50px;
}

    .featured-icon-box.icon-align-before-content .featured-icon, .featured-icon-box.icon-align-before-content .featured-content, .featured-icon-box.icon-align-before-title .featured-title, .featured-icon-box.icon-align-before-title .featured-icon {
        display: table-cell;
        vertical-align: middle;
    }
    .reach-us  h2{
font-size:32px;
font-weight:700;
}
.reach-us h4{
    font-size:24px;
    font-weight:700;
    display: table-cell;
    vertical-align: middle;
}
    .reach-us p {
    display: block;
    font-size: 16px;
    color: #212322;
}

}
.reach-us .address{
display:flex;
}
.reach-us .number{
display:flex;
}
.reach-us .featured-title{
padding-top: 20px;
}
.reach-us img{
border-radius:10px;
}


@media (min-width: 1800px){

    .highlight .slick-list{
        padding-left: 30%!important;
    }
}

@media only screen and (min-width: 769px) and (max-width: 1100px) { 
	.d-tab-none{display:none!important;}
}
  /* END reach us */

</style>
